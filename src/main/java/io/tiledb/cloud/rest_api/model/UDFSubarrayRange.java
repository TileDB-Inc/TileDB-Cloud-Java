...
//          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UDFSubarrayRange` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
//        }
//      }
      // validate the optional field `range_start`
      if (jsonObj.getAsJsonObject("range_start") != null) {
        DimensionCoordinate.validateJsonObject(jsonObj.getAsJsonObject("range_start"));
      }
      // validate the optional field `range_end`
      if (jsonObj.getAsJsonObject("range_end") != null) {
